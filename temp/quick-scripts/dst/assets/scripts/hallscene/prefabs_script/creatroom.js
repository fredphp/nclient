    ...
    if (myglobal && myglobal.socket) {
      ...
      myglobal.socket.createRoom(roomConfigId, function (result, data) {
        if (result === 0) {
          ...
          myglobal.playerData.rate = 1; // 保存重连信息到 localStorage

          myglobal.socket.saveReconnectInfo(); // 🚀【性能优化】跳转到游戏场景（已预加载，应秒进）
